...
class Daycare(db.Model):
    ...
    program_types = db.Column(ARRAY(db.Text), nullable=False, server_default="{}")
    ...
    def get_program_types(self):
        ...
        return self.program_types if self.program_types else []
    
    def set_program_types(self, program_types):
        """Set the list of program types offered by this daycare"""
        self.program_types = program_types
    
    def get_age_groups(self):
        """Get the list of age groups served by this daycare"""
        return self.age_groups if self.age_groups else []
    
    def set_age_groups(self, age_groups):
        """Set the list of age groups served by this daycare"""
    ...
    @staticmethod
    def get_available_program_types():
        """Get all available Canadian government program types"""
        return [
            'daycare',
            'family_day_homes',
            'out_of_school_care',
    ...
    def get_available_age_groups():
        """Get all available Canadian government age groups"""
        return [
            'under_19_months',
            '19_months_to_kindergarten',
            'kindergarten_to_grade_6'
        ]
    
    @staticmethod
    def get_program_type_descriptions():
        """Get descriptions for all program types"""
        return {
            'daycare': 'Licensed child care program providing care to infants, preschool children, and kindergarten children for four or more consecutive hours in each day the program is provided. Daycares may serve children from birth to kindergarten.',
            'family_day_homes': 'Approved program where Jobs, Economy and Trade has entered into an agreement with a family day home agency to coordinate and monitor the provision of child care in the private residence of one or more program educators. Family day home programs do not provide care for more than six children in each private residence. Family day homes may serve children from ages 0-12.',
            'out_of_school_care': 'Licensed child care program providing care to kindergarten and school-aged children in any or all of the following periods: before-and-after school, during the lunch hour, or when school is closed. Out-of-school care programs may serve children from kindergarten until before they are 13 years of age, or a child of 13 or 14 years of age who requires care because of a special need.',
            'preschool': 'Licensed child care program providing care to preschool children and kindergarten children for four or less hours per child in each day that the program is provided. Preschools may serve children who are at least 19 months old to kindergarten.',
            'group_family_child_care': 'Licensed child care program providing care in the private residence of the licence holder to infants, preschool children, kindergarten children, and school-aged children. Group family child care providers may serve children from ages 0-12.'
        }
    
    @staticmethod
    def get_age_group_descriptions():
        """Get descriptions for all age groups"""
        return {
            'under_19_months': 'Under 19 months of age',
            '19_months_to_kindergarten': '19 months old to kindergarten',
            'kindergarten_to_grade_6': 'Kindergarten to Grade 6'
        }
